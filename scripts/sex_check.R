...
parser$add_argument(
  "-s", "--sexcheck",
  help = "PLINK2 .sexcheck file path (required)", required = TRUE
)
parser$add_argument(
  "-m", "--min-male-xf",
  help = "Threshold for minimum male XF (required)", required = TRUE
)
parser$add_argument(
  "-f", "--max-female-xf",
  help = "Threshold for maximum female XF (required)", required = TRUE
)
parser$add_argument(
  "-l", "--log",
...
sexcheck$sex <- ifelse(
  sexcheck$F > min_male_xf,
  "Male",
  ifelse(sexcheck$F < max_female_xf, "Female", NA)
)
...
if (nrow(sexcheck_err) > 0) {
  sexcheck_err$case <- NA
  sexcheck_err$case[is.na(sexcheck_err$SNPSEX)] <- "outside_f_thresh"
  sexcheck_err$case[
    sexcheck_err$PEDSEX == 1 & sexcheck_err$SNPSEX == 2
  ] <- "mislab_male"
  sexcheck_err$case[
    sexcheck_err$PEDSEX == 2 & sexcheck_err$SNPSEX == 1
  ] <- "mislab_female"
